...
Markov.prototype.ask = function(opt) {
    ...
    return _.compact(seed.concat(step(seed,[]))).join(this.globals.seperator); 
    // using compact to remove empty values (which occurs if seed is empty, and seprator is not "") but could be dangerous, as it removes all falsey values - which we may want? (e.g. 0) //
